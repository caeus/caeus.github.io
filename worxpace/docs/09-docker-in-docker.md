...
worxpace runs inside a container but drives the **host's** Docker daemon through a mounted
...
- `/repo` — inside the worxpace container.
...
starts writing into a fresh empty directory inside the ephemeral worxpace container and the
files vanish when it exits. There is no error — the copy succeeds, into nowhere. Keep the two
straight.

## Consequences

**Sibling containers, not children.** Every image worxpace builds lands in the host's image
...
privileged container and own the host. worxpace therefore never lets `package.wx` files reach
...
manual `docker build`s, and worxpace all draw from the same pool. A `docker system prune` wipes
worxpace's cache along with the rest.
...
worxpace/cli.sh run packages/ui#ci#build
```
