...
For contributors to worxpace itself, and for anyone debugging behaviour that the authoring
docs don't explain.
...
- `cache` is a `Map<resolvedPath, vm.SourceTextModule>` shared by every `wx:/` import in the
  repo.
...
Because tags are deterministic, they are also predictable from outside worxpace — handy for
...
socket. Dependencies are installed before `src/` is copied, so editing worxpace's source only
