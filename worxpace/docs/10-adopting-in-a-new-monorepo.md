# Adopting worxpace in a new monorepo

worxpace has no published package. You adopt it by copying the `worxpace/` directory into your
...
```sh
cp -R <source>/worxpace <your-repo>/worxpace
```

You need `Dockerfile`, `cli.sh`, `wx`, `install.sh`, `package.json`, `pnpm-workspace.yaml`,
`pnpm-lock.yaml`, `tsconfig.json`, and `src/`. The directory must be named `worxpace` and sit
at the repo root — the `wx` launcher finds your repository by walking up until it sees a
...
## Local development alongside worxpace

worxpace is for reproducible builds, not for the inner loop. Nothing stops you from keeping a
normal package-manager workspace for day-to-day work and using worxpace for verification and
release. The two can coexist as long as you accept that the host `node_modules` and the
in-image one are separate trees.

If you go this route, treat the `.wx` version registry as authoritative and derive host-side
...
## Adapting worxpace itself

The hardwired conventions in
[07 — Conventions and layout](07-conventions-and-layout.md#hardwired-you-cannot-change-without-editing-worxpace)
