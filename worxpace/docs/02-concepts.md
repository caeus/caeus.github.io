...
There are two layers of caching, and only one of them is worxpace's.
...
**worxpace's memo table** is per-process only. Within one `wx run`, the graph is walked once
...
`package.wx` files are evaluated in a `node:vm` context with no filesystem, no network, and
no `process`. A build file cannot read a file to decide what to do — it can only compute from
its own literals and from other `.wx` modules it imports. That is a constraint, and it is the
point: the target graph is a pure function of the repo's source, so `wx list` gives the same
answer on every machine.

See [04 — The sandbox and `wx:/` imports](04-sandbox-and-imports.md) for exactly what is and
is not available inside a build file.

## What worxpace deliberately does not do
...
- **No watch mode.** For iterating on a dev server, use your package manager directly; worxpace
